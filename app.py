...
st.title("Real-Time Isolated Sign Prediction")
st.write("This app captures video from your webcam, performs ASL (American Sign Language) prediction in real-time, and displays the result.")
...
def real_time_asl(feed_placeholder):
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cap.isOpened():
            ...
            if len(sequence_data) % 15 == 0:
                ...
                result_text = decoder(sign)
                st.write(f"Predicted Sign: {result_text}")

            feed_placeholder.image(image, channels="BGR", use_column_width=True)
...
# Streamlit button to start the real-time ASL prediction
if col1.button("Start ASL Prediction"):
    real_time_asl(feed_placeholder) 
